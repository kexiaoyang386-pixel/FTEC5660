...
def _get_openai_client() -> OpenAI:
    ...
    if openai_client is None:
        if not OPENAI_API_KEY:
            raise ValueError(
                "OPENAI_API_KEY is not set. Set it in your environment before running evaluation."
            )
...
LLM_STATS = {"calls": 0, "seconds": 0.0}


def reset_llm_stats() -> None:
    LLM_STATS["calls"] = 0
    LLM_STATS["seconds"] = 0.0
...
def _spider_root() -> str:
    env = os.getenv("SPIDER_ROOT")
    if env:
        return env
    local = _repo_path("spider_data")
    if os.path.isdir(local):
        return local
    legacy = os.path.normpath(_repo_path("..", "..", "spider"))
    if os.path.isdir(legacy):
        return legacy
    return local


def _spider_json_path(filename: str) -> str:
    root = _spider_root()
    candidates = [
        os.path.join(root, filename),
        _repo_path(filename),
        _repo_path("spider_data", filename),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
...
def _spider_db_path(db_id: str) -> str:
    root = _spider_root()
    candidates = [
        os.path.join(root, "database", db_id, f"{db_id}.sqlite"),
        _repo_path("spider_data", "database", db_id, f"{db_id}.sqlite"),
        os.path.normpath(_repo_path("..", "..", "spider", "database", db_id, f"{db_id}.sqlite")),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
...
def load_local_model(model_id="Qwen/Qwen2.5-1.5B-Instruct"):
    ...
    if torch is None or AutoTokenizer is None or AutoModelForCausalLM is None:
        raise ModuleNotFoundError(
            "Local model requires 'torch' and 'transformers'. Install them or avoid calling load_local_model()/call_agent_local()."
        )
...
def query_execution(item, sql):
    ...
    db_path = _spider_db_path(db_id)
    gold_rows, gold_err = exec_query(db_path, gold_sql)
    gen_rows, gen_err = exec_query(db_path, sql)
    ...
    try:
        ...
        if client is None:
            if anthropic is None:
                raise ModuleNotFoundError(
                    "anthropic is not installed. Install it or pass an OpenAI model name (e.g. --model gpt-4o-mini)."
                )
            raise ValueError(
                "ANTHROPIC_API_KEY is not set, but call_agent() was invoked without an OpenAI model."
            )
        ...
        LLM_STATS["calls"] += 1
        LLM_STATS["seconds"] += time.perf_counter() - start
...
def load_spider(dev: bool = True, testing=False) -> List[Dict]:
    if testing is True:
        path = _repo_path("testing_limit.json")
    else:
        path = _spider_json_path("dev.json" if dev else "train_spider.json")
    with open(path, encoding="utf-8") as f:
        return json.load(f)


def load_schema_without_PKFK(db_id: str) -> str:
    db_dir = os.path.dirname(_spider_db_path(db_id))
...
# NL2SQL bugs file
_bugs_db_path = os.path.join(os.path.dirname(__file__), "nl2sql_bugs.json")
if os.path.exists(_bugs_db_path):
    BUGS_DB = open(_bugs_db_path, encoding="utf-8").read()
else:
    BUGS_DB = ""
